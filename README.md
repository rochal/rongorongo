...
In print mode the key O lays Barthel's tracing of the selected box's line over the photograph in translucent red, turned to match the line's orientation and scaled to its extent, so that a doubtful box can be judged against the drawing without leaving the print; the overlay is nudged with I, J, K and L, rescaled with the bracket keys, and its alignment is saved with the boxes.
