...
| Width of the print glyph against the tracing glyph, relative to line medians | correlation 0.89 over 3,322 glyphs |

**Barthel's proportions are faithful.** The width of each print glyph, measured from its own ink inside a padded box, follows the width of the tracing glyph with a correlation of 0.89. The box comes from the tracing, so the test is not free, but the padding lets the print's ink run a third wider or narrower than the box, and it does not.

**The narrowing along lines is the carver's, not the tracer's.** The section 25 slopes were recomputed on the print glyphs. Over the eleven sides whose glyphs stand at least 30 pixels tall on the print, print and tracing slopes correlate at 0.79 and agree in sign on ten. Aruku Kurenga narrows on both faces in both sources; Keiti's verso and Small Santiago's verso widen in both. The exceptions are the Great Santiago faces, Great Washington and the Great St Petersburg verso, whose prints are so small that a glyph is fourteen to thirty pixels tall, and the Great St Petersburg recto, which narrows on the tracing and not on the print. The small prints are below the resolution where a width can be measured, and the tracing result stands for them unconfirmed.

![Print against tracing: narrowing slope per side, and stroke weight per side](docs/img/prints.png)

**The hands test is blind, and now we know by how much.** The cross-side penalty of section 25 was recomputed on the print glyphs and is again near zero for every pair of sides, between -0.007 and +0.038. Before reading that as one hand, the test was given a positive control: side B's glyphs re-read with every stroke thickened by one pixel. The control penalties are indistinguishable from the real ones, median 0.012 against 0.011 on the prints and 0.015 against 0.011 on the tracings. The descriptor, a blurred silhouette with gradient histograms at 32 pixels, does not see stroke weight, which is the first thing a different tool or hand would change. The near-zero penalties in section 25 and here are therefore not evidence of one hand; they are evidence that this descriptor cannot tell hands apart. That is a correction to the way section 25 put it.
