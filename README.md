...
Rongorongo is undeciphered and this report does not change that. It records structural results, twenty-nine sections of them, obtained by scripting over the published numerical transliteration, Barthel's sign catalogue and tracings, the retouched photographs of the tablets, and the Rapa Nui, Māori and Tahitian texts that exist, each with its evidence and its caveat, so that they can be checked against the literature and built on. Some results were retired or weakened along the way as the checks improved; the report keeps the record of that rather than the first version. Every result is reproducible from the scripts in this repository.
...
- **The Mamari calendar rebuilds itself from the transcription,** eight marker groups around 28 crescents, and a natural way of counting the other signs among them gives exactly thirty nights. Its two longest runs match the two stretches of the month that share a name, but when the thirty nights are laid against the named month the markers fall on the boundaries between name groups no more often than chance would put them, and the crescents do not change their facing between the waxing and the waning half, so nothing anchors the calendar to a named night. Section 21.
...
- **The drawings have been checked against the wood, and the checking changed the results.** Ten thousand glyphs were cut from Barthel's tracings, the tracings were registered to the retouched photographs line by line, and one side, Keiti's verso, was boxed by hand on the photograph to serve as ground truth. That ground truth retired two claims. A first version found carvers narrowing their glyphs toward the ends of lines on most tablets; a fault in how the drawings were cut, plain strokes split into slivers, was producing much of it, and after the fix the narrowing remains on a handful of sides, while Keiti's widening survives every check. A test that had found Barthel's proportions faithful turned out to be circular, and on the hand-boxed lines his widths agree with the wood at only 0.65. The automatic boxes, scored against the hand ones, are good where the drawing's units are right and wrong together with it where they are not. Whose hand carved what stays open: the shape matcher cannot see stroke weight, and in a retouched print the stroke is the retoucher's. Sections 9 and 25.
...
| Are Barthel's tracings faithful to the wood? | Open; the automatic width test was circular, on hand-boxed lines his widths agree at 0.65, and the drawings split strokes and join figures the carver did not | 25 |
| Are the rare catalogue numbers variants of common signs? | Not decidable by context in this corpus; candidates from the hand pass are logged for shape tests on the prints | 7 |
...
**The automatic registration, scored against the hand boxes.** With seven lines of Keiti's verso boxed by hand, the automatic placement can be scored: the same side registered with the corrections ignored, each automatic box snapped to the ink it keeps, and compared with the hand box for the same unit. The score drove two changes to the placement: the map from tracing to print along a line is now a median of slopes over the matched chunks, re-estimated after a first pass from the glyphs that matched well and applied in a second pass with no carried shift, so that a drift cannot propagate; and a line with too few matched chunks borrows the side's median stretch, since Barthel's scale is one per side. The boxes are also snapped to the ink, which fixed their size: before, they were 29 percent narrower than the hand boxes.

| | Glyphs | Median overlap (IoU) | Overlap at least 0.5 | At least 0.7 | Median centre offset | Offset over glyph width |
|---|---|---|---|---|---|---|
| All seven lines | 269 | 0.45 | 49% | 29% | 12 px | 0.28 |
| Ev03, Ev04, Ev06 | 119 | 0.65 to 0.75 | 78% | | 3 px or less | |
| Ev02 | 29 | 0.47 | | | 5 px | |
| Ev05, Ev07, Ev08 | 121 | 0.00 to 0.03 | | | 45 to 89 px | |

The split by line is the finding. On the three lines where the two box sets agree about which drawing is which unit, the placement is good: three quarters of the boxes overlap their glyph by more than half and the typical box is within three pixels of centre. Ev02 is between. On Ev05, Ev07 and Ev08 the automatic boxes sit a glyph or more from the hand boxes along most of the line, and the offset profile shows why: on Ev05 the boxes agree up to position 13 and then run a glyph behind to the end. That is not a placement error. The registration maps the tracing's count-aligned boxes onto the print, and on those lines the tracing's count matched the transliteration by coincidence, one blob for two touching glyphs and two blobs for one glyph elsewhere, so the tracing's unit numbers are wrong between the two points and every print box inherits it. On Ev08 the hand boxes also span 17 percent less of the print than Barthel's line, at its worn end. Relative glyph widths from the automatic boxes correlate with those from the hand boxes at 0.60 over all glyphs and far higher on the clean lines.

What this bounds: on a line whose tracing alignment is right, the automatic print boxes are usable as measurements; on a line where it is wrong by coincidence, which no automatic check detects, they are wrong together. The print-based numbers on the other sides therefore carry both errors, and a confirmation from an automatic side remains weaker than one from Keiti's hand boxes. Fixing the second error needs hand boxes on the tracing, which the editor now supports, or a segmentation that does not trust a matching count. The score is reproduced by `scripts/registration_score.py`, with a per-line table in `out/registration_score.md`.
...
The scripts, this README, the generated outputs and the hand-drawn boxes in `data/boxes` are under the MIT licence in LICENSE. The downloaded transliteration and sign catalogue are not redistributed here and remain with the CEIPP; the line crops in docs/img are from Barthel's tracings as hosted on Wikimedia Commons, the check images in section 25 show Chauvet's 1935 print of Keiti as hosted there, and the small glyph strips in docs/img/glyphs are cut from Barthel's catalogue drawings as reproduced on kohaumotu.org, included as quotations for the purpose of commentary. The Māori and Tahitian texts are public domain and are fetched, not stored.

### Reproduction

The sequence was run end to end from a fresh clone twice. The second run, on 7 September 2026 at commit 2dcef41: `pip install -r requirements.txt` and `python run_all.py`, 57 steps in 90 minutes on a 32-core machine, every data source fetched again, the prints and the Māori and Tahitian texts included. `scripts/check_reproduction.py` then compared the regenerated `out/` with the committed one: 137 of 157 tables identical byte for byte, the registration of all 22 prints and its 5,414 glyph boxes among them. Nineteen of the twenty that differ follow from the Métraux page scans, which cannot be redistributed and so are absent from a clone: without them the Rapa Nui corpus is Thomson's recitations alone, and every table downstream of that corpus, the word comparison, the attachment profile, the matched-size statistics and the four decipherment runs, takes the values a Thomson-only corpus gives. The twentieth was a fault in the run order, the calendar's crescent-orientation measure running before the tracings it measures had been cut, fixed in the run list at once. Anyone who adds the scans to `data/metraux/` reproduces the committed numbers. The first run, on 6 September over the 40 steps that then existed, had found 113 of 122 tables identical with the same nine Métraux-dependent exceptions.
...
- Chauvet, S. 1935. *L'île de Pâques et ses mystères.* Paris. The white-filled prints of the tablets, as hosted on Wikimedia Commons (files Rongorongo <side> <name>.jpg), fetched through the Commons API; not redistributed here, the boxes cut from them are.
- Guy, J. B. M. 1990. On the lunar calendar of Tablet Mamari. *Journal de la Société des Océanistes* 91.
- Māori texts, public domain, OCR from the Internet Archive: *Ko te Paipera Tapu* 1868 (kotepaiperatapua00barl), *Ko te Kawenata Hou* 1841 (kotekawenatahou00yategoog), Grey, G. *Ko nga mahinga a nga tupuna Maori* 1854 (kongamahingaang00greygoog) and *Ko nga moteatea* 1853 (kongamoteateame00greygoog).
- Tahitian texts, public domain, OCR from the Internet Archive: *Te Bibilia Moa Ra* 1878 (tebibiliamoaraoi00lone) and *Te Faufaa Api* 1853 (tefaufaaapiatot00unkngoog).
- Hand-drawn glyph boxes and the discrepancy log: `data/boxes/`, made with `scripts/box_editor.py` in this repository.
